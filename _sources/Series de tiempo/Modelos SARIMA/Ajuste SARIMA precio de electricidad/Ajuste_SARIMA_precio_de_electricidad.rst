Ajuste SARIMA precio de electricidad
------------------------------------
...
.. raw:: html
      ...
      <div id="df-137409c5-f727-4bdd-baf7-4771374a2ffe" class="colab-df-container">
        <div>
    <style scoped>
    ...
    <table border="1" class="dataframe">
      ...
      <div class="colab-df-container">
        <button class="colab-df-convert" onclick="convertToInteractive('df-137409c5-f727-4bdd-baf7-4771374a2ffe')"
      ...
      <style>
        ...
        <script>
          const buttonEl =
            document.querySelector('#df-137409c5-f727-4bdd-baf7-4771374a2ffe button.colab-df-convert');
          buttonEl.style.display =
            google.colab.kernel.accessAllowed ? 'block' : 'none';
    
          async function convertToInteractive(key) {
            const element = document.querySelector('#df-137409c5-f727-4bdd-baf7-4771374a2ffe');
            const dataTable =
        ...
        <div id="df-2896354e-8fb5-408e-a4c0-51cb32d0b297">
          <button class="colab-df-quickchart" onclick="quickchart('df-2896354e-8fb5-408e-a4c0-51cb32d0b297')"
          ...
          <script>
            ...
            (() => {
              let quickchartButtonEl =
                document.querySelector('#df-2896354e-8fb5-408e-a4c0-51cb32d0b297 button');
...
El procedimiento auto-ARIMA seleccionó ARIMA(3,1,3) como especificación
óptima bajo el criterio AIC. El examen de la ACF/PACF de la serie
Box-Cox con d=1 muestra un pico estacional en :math:`s=12` de magnitud
moderada, compatible con estacionalidad mensual.

En consecuencia, es apropiado contrastar extensiones estacionales del
modelo mediante :math:`SARIMA(3,1,3)(1,0,0)_{12}`,
:math:`SARIMA(3,1,3)(0,0,1)_{12}` y :math:`SARIMA(3,1,3)(1,0,1)_{12}`,
comparando desempeño y diagnóstico de residuales.

SARIMA(3,1,3)(1,0,0,12)
~~~~~~~~~~~~~~~~~~~~~~~
...
.. code:: ipython3
    ...
    seasonal_order = (1, 0, 0, 12)  # Parámetros de la estacionalidad (P, D, Q, s)
...
.. parsed-literal::
    ...
    Model:             SARIMAX(3, 1, 3)x(1, 0, [], 12)   Log Likelihood                 411.133
    Date:                             Fri, 07 Nov 2025   AIC                           -806.266
    Time:                                     21:04:10   BIC                           -778.727
    Sample:                                 01-01-2000   HQIC                          -795.159
                                          - 04-01-2019                                         
    Covariance Type:                               opg                                         
    ==============================================================================
                     coef    std err          z      P>|z|      [0.025      0.975]
    ------------------------------------------------------------------------------
    ar.L1          1.7560      0.137     12.859      0.000       1.488       2.024
    ar.L2         -1.6882      0.149    -11.295      0.000      -1.981      -1.395
    ar.L3          0.7459      0.101      7.368      0.000       0.547       0.944
    ma.L1         -1.8682      0.147    -12.748      0.000      -2.155      -1.581
    ma.L2          1.6910      0.209      8.077      0.000       1.281       2.101
    ma.L3         -0.7611      0.110     -6.901      0.000      -0.977      -0.545
    ar.S.L12      -0.0107      0.065     -0.165      0.869      -0.137       0.116
    sigma2         0.0017      0.000     10.945      0.000       0.001       0.002
    ===================================================================================
    Ljung-Box (L1) (Q):                   0.17   Jarque-Bera (JB):                 1.96
...
**Se observa una ligera significancia en el rezago 13**

SARIMA(3,1,3)(0,0,1,12)
~~~~~~~~~~~~~~~~~~~~~~~
...
.. code:: ipython3
    ...
    order = (3, 1, 3)  # Puedes ajustar según el análisis de ACF y PACF
    trend = 'n'        # 'c' = constante, 't' = tendencia, 'ct' = constante + tendencia, 'n' = sin tendencia
    seasonal_order = (0, 0, 1, 12)  # Parámetros de la estacionalidad (P, D, Q, s)
...
.. parsed-literal::
    ...
    ============================================================================================
    Dep. Variable:                        Precio_boxcox   No. Observations:                  232
    Model:             SARIMAX(3, 1, 3)x(0, 0, [1], 12)   Log Likelihood                 411.683
    Date:                              Fri, 07 Nov 2025   AIC                           -807.367
    Time:                                      21:04:23   BIC                           -779.828
    Sample:                                  01-01-2000   HQIC                          -796.259
                                           - 04-01-2019                                         
    Covariance Type:                                opg                                         
    ==============================================================================
                     coef    std err          z      P>|z|      [0.025      0.975]
    ------------------------------------------------------------------------------
    ar.L1          1.8172      0.088     20.761      0.000       1.646       1.989
    ar.L2         -1.7691      0.098    -18.001      0.000      -1.962      -1.577
    ar.L3          0.8035      0.069     11.710      0.000       0.669       0.938
    ma.L1         -1.9417      0.096    -20.323      0.000      -2.129      -1.754
    ma.L2          1.8184      0.150     12.130      0.000       1.525       2.112
    ma.L3         -0.8393      0.081    -10.405      0.000      -0.997      -0.681
    ma.S.L12       0.0113      0.063      0.180      0.857      -0.112       0.135
    sigma2         0.0017      0.000     10.744      0.000       0.001       0.002
    ===================================================================================
...
**Se identifica una leve significancia en el rezago 13.**
...
**Se sigue observando una leve significancia en el rezago 13.
Adicionalmente, el rezago 8 también muestra una significancia marginal.
Por lo tanto, se ajustará el modelo SARIMA(8,1,8)(1,0,1,12) con el
objetivo de que, en el análisis de los residuales, desaparezcan los
rezagos significativos y así se logre un modelo que capture de manera
adecuada toda la estructura temporal de la serie.**
...
**Con este modelo, el análisis de los residuales confirma la ausencia de
autocorrelaciones significativas, evidenciando un ajuste adecuado a la
